...
namespace Cognex.DataMan.SDK
{
    internal class GuiLogger : ILogger
    {
        ...
        private void DisplayEnqueuedMessages()
        {
            ...
            if (message != null)
            {
                if (_logBox.InvokeRequired)
                    _logBox.BeginInvoke(new LogHandler(LogToGui_Invoke), new object[] { message });
                else
        ...
        {
            System.Diagnostics.Debug.Assert(_logBox.InvokeRequired == false);   // Must be called on the GUI thread
